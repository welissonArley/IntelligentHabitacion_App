...
{
    public class SendResetPasswordHelper : SendEmailHelper, ISendResetPasswordEmail
    {
        private readonly ICustomRazorEngine _customRazorEngine;
        private ResetPasswordDto _dto;

        public SendResetPasswordHelper(ICustomRazorEngine customRazorEngine, ISendEmail sendEmail) : base(customRazorEngine, sendEmail)
        {
            _customRazorEngine = customRazorEngine;
        }

        public async Task Send(ResetPasswordDto dto)
        {
            _dto = dto;

            var resetPasswordModel = GetResetPasswordModel();

            string resetPasswordBody = await _customRazorEngine.RazorViewToHtmlAsync("Areas/MyFeature/Pages/ResetPasswordEmailBody.cshtml", resetPasswordModel);
